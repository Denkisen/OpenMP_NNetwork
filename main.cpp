...
//#pragma omp parallel for reduction(+:sum)
  ...
  try 
  {
    ...
    // Vulkan::Array<unsigned> input_image(device, inp);
    // Vulkan::Array<unsigned> output_image(device, out);
    // std::vector<Vulkan::Array<unsigned>*> data;

    // data.push_back(&input_image);
    // data.push_back(&output_image);1
    // Vulkan::Offload<unsigned> offload(device, data, "bin/comp.spv");
    // STOP_WATCH(start);
    // offload.Run(input.width / 32, input.height / 32, 1);
    // STOP_WATCH(end);
    // PRINT_DIFF_WATCH(start, end);
    // out = output_image.Extract();
